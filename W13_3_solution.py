import csv

temperatures = []
num_of_lines_to_process = 10000
biggestNumber:float
biggest_value:int
smallest_number:float
smallest_value:int
with open('temperature_partial.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    for row in csv_reader:
        if line_count == 0:
            print(f'Column names are {", ".join(row)}')
            line_count += 1
            
        else:
          #Set biggest number to the first row of data
          if(line_count == 1):
            biggestNumber = float(row[1])
            smallest_number = float(row[1])

          #print(f'Pressure: \t{row[1]}')
          temperatures.append([row[1],row[3]])
          #If the current value of the data is bigger than the previous biggestNumber, set biggestNumber equal to the new data value.
          
          if(biggestNumber<float(row[1])):
            biggestNumber = float(row[1])
            biggest_value = line_count

          
          if(smallest_number>float(row[1])):
            smallest_number = float(row[1])
            smallest_value = line_count
          line_count += 1
          
          #if(line_count > num_of_lines_to_process):
          #  break
    print(f'Processed {line_count} lines.')
#print(smallerNumber)
print(f'The highest temperature is {biggestNumber} at {temperatures[biggest_value-1][1]}' )
print(f'The lowest temperature is {smallest_number} at {temperatures[smallest_value-1][1]}' )
